...
const AddOrgPkg = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      const res = await axios.get(`/package/student/${id}/details`);
      setPackData(res?.data);
    };
    router?.isReady && id && getData();
  }, [id, router?.isReady]);
  return (
    <Layout title={`Student Package / ${id ? `#${id}` : "New"}`} back>
      <StudentPackForm packData={packData} />
    </Layout>
  );
};
...
const StudentPackForm = ({ packData }) => {
  ...
  const onSubmit = async (data) => {
    if (image || packId) {
      ...
      try {
        ...
        if (packData) {
          const res = await axios.put(`/package/student/${packId}`, formData, {
            headers: {
              "Content-Type": "multipart/form-data",
            },
          });
  ...
  useEffect(() => {
    if (packData) {
      ...
      setValue("cost", packData?.cost);
      setValue("pre_price", packData?.pre_price);
      setImageSrc(packData?.thumbnail);
      reset({ text: packData?.text });
    }
