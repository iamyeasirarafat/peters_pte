...
const ListTable = ({ data }) => {
    ...
    return mounted && isTablet ? (
        ...
        <table className="bg-white dark:bg-black w-full">
            ...
            <tbody>
                {data?.map((item) => (
                    <TransactionsHistoryRow data={item} listName={ListName} key={item.id} />
                ))}
            </tbody>
        </table>
    );
...
const TransactionsHistoryRow = ({ data, listName }) => {
    return (
        <tr>
            <td className="td-custom">
                <div className="flex items-center gap-x-3">
                    <Image
                        ...
                        alt=""
                    />
                    <p className="text-sm font-bold dark:text-white">{data?.title || "N/A"}</p>
                </div>
            </td>
            <td className="td-custom text-sm font-bold text-right">N/a</td>
            <td className="td-custom text-right">
                {listName === "students_package" ? `${data?.validity} Days` : data?.variation}
            </td>
    ...
    <div className="flex items-center justify-between p-4">
        <div className="flex items-center gap-x-3 ">
            ...
            <div className="space-y-1">
                ...
                <p className="text-[#5F646D] dark:text-white text-sm font-medium">Stripe</p>
            </div>
        </div>
        <div className="text-right space-y-1">
            <p className="text-sm font-bold">5940 BDT</p>
            <p className="text-[#5F646D] dark:text-white text-sm font-medium">#PPTE - 548602</p>
        </div>
    </div>
);
