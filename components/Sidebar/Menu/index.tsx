...
const Menu = ({ visible }: MenuProps) => {
  ...
  return (
    <>
      <div
        className={`mb-3 overflow-hidden whitespace-nowrap text-xs font-medium text-white/50 ${visible ? "w-full opacity-100" : "xl:w-full xl:opacity-100 w-0 opacity-0"
          }`}
      >
  ...
  return (
    <div className="-mx-4 mb-10">
      {menu.map((link: any, index: number) => (
        <Link
          className={twMerge(
            `flex items-center h-9.5 mb-2 px-4 text-sm text-white fill-white font-bold last:mb-0 transition-colors hover:bg-[#161616] ${router.pathname === link.url && "bg-[#161616] text-primary"
        ...
        >
          <Icon
            className={`mr-3 ${router.pathname === link.url ? "fill-primary" : "fill-inherit"
